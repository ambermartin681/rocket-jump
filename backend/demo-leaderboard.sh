...
echo "Ã°Å¸Å¡â‚¬ Token Leaderboard API Demo"
echo "================================"
echo ""

# Check if server is running
echo "Ã°Å¸â€œÂ¡ Checking server status..."
if ! curl -s "${BASE_URL}/health" > /dev/null 2>&1; then
    echo "Ã¢ÂÅ’ Server is not running at ${BASE_URL}"
    echo "Please start the server with: npm run dev"
    exit 1
fi
echo "Ã¢Å“â€¦ Server is running"
echo ""
