...
async function runTests() {
  ...
  console.log("Ã¢Å“â€¦ Token created:", token.id);

  // 3. Read token
  const found = await getTokenByAddress("0xTEST_ADDRESS");
  console.log("Ã¢Å“â€¦ Token found:", found?.name);
  ...
  console.log("Ã¢Å“â€¦ Burn record created:", burn.id);

  // 5. Cleanup
  await prisma.burnRecord.delete({ where: { id: burn.id } });
  await prisma.token.delete({ where: { id: token.id } });
  console.log("Ã¢Å“â€¦ Cleanup complete");
